...
class Derain(object):
    ...
    def generate_data(self,data_dir):
        ...
        for j,i in enumerate(os.listdir(data_dir)):
            ...
            ig=imageio.imread(os.path.join(data_dir,i))
            print(i)
            norain_=ig[:,:ig.shape[1]//2,:]
            rain_=ig[:,ig.shape[1]//2:,:]
            if(norain_.shape[0]>=256 and norain_.shape[1]>=256):
                norain_=np.array(Image.fromarray(norain_).resize(((256,256))))
                rain_=np.array(Image.fromarray(rain_).resize(((256,256))))
            elif(norain_.shape[0]<256 and norain_.shape[1]>256):
                norain_= np.array(Image.fromarray(norain_).resize(((norain_.shape[0],256))))
              #  //norain_=misc.imresize(norain_,((norain_.shape[0],256,3)))
                rain_ = np.array(Image.fromarray(rain_).resize(((norain_.shape[0],256))))
                ...
                norain_=np.array(Image.fromarray(norain_).resize(((norain_.shape[1],256))))
                rain_=np.array(Image.fromarray(rain_).resize(((norain_.shape[1],256))))
                h=0
